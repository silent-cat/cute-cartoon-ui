<template>
  <div class="calendar" :class="theme">
    <div class="tools">
      <span class="leftArr" @click="handlePrevM()">&lt;</span>
      <div class="center" @click="toggleSelYear">
        <span class="year">{{ thisY }}</span>
        <span>-</span>
        <span class="month">{{ thisRM }}</span>
      </div>
      <span class="rightArr" @click="handleNextM()">&gt;</span>
      <div class="selyear-wrap" v-show="showSelYear">
        <span
          class="sel-span"
          v-for="(item, index) in 25"
          :key="index + 'y'"
          @click="handleSelYear(thisY - 12 + index)"
          :class="{ current: thisY == thisY - 12 + index }"
          >{{ thisY - 12 + index }}</span
        >
      </div>
    </div>
    <div class="weeks">
      <span class="week" v-for="(item, index) in weeks" :key="index">{{
        item
      }}</span>
    </div>
    <div class="days" @mouseleave="handleMouseLeave">
      <span
        class="prevMD sel-span"
        v-for="(item, index) in prevMDLen"
        :key="index + 'p'"
        @click="handleSelPrevD(monthDays[prevM] - prevLD[thisW] + item)"
        >{{ monthDays[prevM] - prevLD[thisW] + item }}</span
      >
      <span
        class="thisMD sel-span"
      ...
      >
      <span
        class="nextMD sel-span"
        v-for="(item, index) in nextMDLen"
        :key="index + 'n'"
        @click="handleSelnextD(index + 1)"
        >{{ index + 1 }}</span
      >
    </div>
    <div class="btns">
    ...
    thisD() {
      if (this.selDay) {
        if (this.selMode == 'mutiSel') {
          this.selDayList.push({
            year: this.selDay.getFullYear(),
            month: this.selDay.getMonth(),
            day: this.selDay.getDate()
          })
        }
        if (this.selMode == 'rangeSel') {
  ...
  methods: {
    ...
    handleSelCurD(index) {
      ...
      if (this.selMode == 'mutiSel') {
        this.selDayList = this.selDayList.filter(item => {
          // debugger
          if (
            item.year == _that.thisY &&
            item.month == _that.thisM &&
            item.day == _that.thisD
          ) {
            this.selDay = ''
            return false
          }
    ...
    showSelected(index) {
      ...
      this.selDayList.forEach(item => {
        if (
          item.day == index &&
          item.month == this.thisM &&
          item.year == this.thisY
        ) {
          sel = true
        }
      })
    ...
    showRange(index) {
      ...
      return (
        ...
        index < endIndex &&
        index > startIndex &&
        (this.mouseEnterActive || this.selDayList.length == 2)
